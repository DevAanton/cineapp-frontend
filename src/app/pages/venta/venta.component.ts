...
export class VentaComponent implements OnInit {
  ...
  seleccionarAsiento(asiento: number) {
    ...
    //console.log(this.precioEntrada);
  ...
  seleccionarComida(e: any, c: Comida) {
    if (e.checked) {
      ...
      this.precioTotal += c.precio;

     console.log("seleccion precio total");
     console.log(this.precioTotal);
    
    } else {      

      let removeIndex = this.comidasSeleccionadas.findIndex(itm => itm.idComida===c.idComida);

      if(removeIndex !== -1)
        this.comidasSeleccionadas.splice(removeIndex,1);

      this.precioTotal = this.precioTotal - c.precio;

      // console.log("desseleccion precio total");
      // console.log(this.precioTotal);  
    }
  ...
  sumarPrecioComida(arr: Comida[]= []): number{
    var total_suma = 0;     
    arr.forEach(function (comida) {        
        total_suma += (+comida.precio);
    });   
    return total_suma;
  }  
